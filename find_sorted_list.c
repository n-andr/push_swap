...
extract_sorted_list:
	This function extracts the longest increasing sequence from stack a to stack b.
	It uses the previous two functions to find and mark the longest sequence.
	Then, it moves the unmarked nodes to stack b, leaving the sorted sequence in stack a.
*/
...
void	extract_sorted_list(t_stack **a, t_stack **b)
{
	...
	start_value = find_sorted_list(*a, *a, *a);
